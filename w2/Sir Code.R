...
write.csv(fullData,"test_final.csv")
...
bp=sprintf("bnTime_%d", 0:60)
colnames(fullData)[126:186]=bp
write.csv(fullData,"train_wi1.csv")

setwd()
temp = fread("E:/USA/Projects/Research/Conferance/Individual_subjects/Individual_subjects/A_M1012YM06_1S3RNA.csv")[,c("Event_ID","RunName","TD","win60s","Time","Steering_Angle","Brake_Pedal_Pos","Acceler_Pedal_Pos")]
temp = arrange(temp, Time)
temp = group_by(temp, RunName, win60s,Event_ID)
#temp$Time = round(temp$Time, 1)
temp$nTime = floor(temp$Time*1)
