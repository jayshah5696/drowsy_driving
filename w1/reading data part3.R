...
#example=bulk(directory = "E:/USA/Projects/Research/Conferance/dataset/example", subdirectories = FALSE, extension = ".csv", data = NULL, verbose = F, fun = data.table::fread)
               ...
               data = NULL, verbose = TRUE, fun = utils::read.csv, ...) 
{
  ...
  for (subdirectory in subdirectories) {
    ...
    for (file in files) {
      ...
      single_data$File <- file
      #case310=single_data[single_data=='309']
      #max310=single_data[single_data$win30s]
      #ase310=single_data[single_data$win30s==max310]
      #Exytracting 311 cases and since, first and last windows are incomplete, we are simply removing it(data drain)
      single_data=single_data[single_data$Event_ID=='311',]
      #single_data=rbind(case310,single_data)
      xmin=min(single_data$win30s)
      single_data=single_data[single_data$win30s!=xmin]
               ...
               data = NULL, verbose = TRUE, fun = utils::read.csv, ...) 
{
  ...
  for (subdirectory in subdirectories) {
    ...
    for (file in files) {
      ...
      single_data=single_data[single_data$Event_ID=='311',]
      #single_data=rbind(case310,single_data)
      xmin=min(single_data$win10s)
      single_data=single_data[single_data$win10s!=xmin]
