...
features = function(data){
    ...
    for (i in c(1:dim(data)[1])) {
        ...
        entropy_speed[i,] = approx_entropy(unlist(data[i , 5:65]), edim = 2,  elag = 1)
        entropy_acc_lot[i,] = approx_entropy(unlist(data[i , 66:126]), edim = 2, elag = 1)
        entropy_acc_lat[i,]= approx_entropy(unlist(data[i , 127:187]), edim = 2, elag = 1)
    }
    newdata =as.data.table(cbind(mean_speed,mean_acc_lot,mean_acc_lan, sd_speed,sd_acc_lot,sd_acc_lat,
                                 ...
                                 entropy_speed,entropy_acc_lot, entropy_acc_lat))
    colnames(newdata) = c("mean_speed","mean_acc_lot","mean_acc_lan","sd_speed","sd_acc_lot","sd_acc_lat",
                          "max_speed",
...
```{r Building Clust}
preprocessParams <- preProcess(train_feat, method=c("scale"))
train_feat1 <- predict(preprocessParams, train_feat)
hc_ward=hclust(dist(train_feat1), method="ward.D")
```
...
fviz_nbclust(train_feat1, hcut, method = "wss",hc_method = "ward.D", main = "Ward.D") +
